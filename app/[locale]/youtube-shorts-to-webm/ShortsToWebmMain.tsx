...
import {
  SHORTS_TO_WEBM_FAQ_ITEMS,
  SHORTS_TO_WEBM_LAST_MODIFIED,
...
const TYPE = {
  badge: "text-xs font-semibold uppercase tracking-[0.12em] text-teal-200",
  ...
  stepNum:
    "flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-teal-500/20 text-sm font-bold text-teal-200 md:h-11 md:w-11 md:text-base",
  ...
  link: "text-sm font-medium text-teal-300 transition hover:text-teal-200 md:text-base",
} as const
...
export default function ShortsToWebmMain() {
  const t = useTranslations("ShortsToWebmPage")
        ...
        <div className={`${SECTION} py-7 md:py-8`}>
          <div className="flex flex-col gap-4">
            <header className="text-center">
              <p
                className={`inline-flex max-w-full items-center gap-2 rounded-full border border-teal-400/30 bg-teal-500/10 px-3 py-1.5 md:px-3.5 ${TYPE.badge}`}
              >
              ...
              <h1
                id="hero-title"
                className={`mt-3 bg-gradient-to-r from-teal-200 via-slate-100 to-emerald-300 bg-clip-text md:mt-4 lg:mt-5 ${TYPE.h1}`}
              >
                {t("hero_title")}
              </h1>
              ...
              <section
                className="fact-chunk mx-auto mt-4 w-full max-w-lg rounded-2xl border border-teal-500/20 bg-teal-500/5 p-4 text-left md:mt-5 md:max-w-none md:p-5"
                aria-label="Quick answer"
              >
                <p className={TYPE.body}>
                  <strong className="font-semibold text-teal-200">{t("quick_answer_label")}</strong>{" "}
                  {t("quick_answer_text")}
                </p>
                ...
                <p className={`mt-3 md:mt-4 ${TYPE.meta}`}>
                  <time dateTime={SHORTS_TO_WEBM_LAST_MODIFIED}>
                    {t("content_updated")}: {SHORTS_TO_WEBM_LAST_MODIFIED}
                  </time>
                </p>
              </section>
            </header>
            ...
            <div className="w-full min-w-0 lg:col-span-7">
              <ShortsDownloader variant="hero" autoFocus shortsType="webm" />
            </div>
          </div>
        </div>
      </section>
        ...
        <div className={`${SECTION} ${SECTION_PY}`}>
          ...
          <div className="mt-5 grid grid-cols-1 gap-3 md:mt-7 md:grid-cols-2 md:gap-4 lg:mt-8 lg:gap-5">
            {WHY_ITEMS.map(({ icon: Icon, titleKey, bodyKey }) => (
              <article
                key={titleKey}
                className="rounded-2xl border border-teal-500/20 bg-gradient-to-br from-teal-500/10 to-slate-900/50 p-4 md:p-5"
              >
                <Icon className="mb-3 h-6 w-6 text-teal-300" strokeWidth={2} aria-hidden />
          ...
          <div className="mx-auto mt-5 grid max-w-xl grid-cols-1 gap-3 md:mt-7 md:max-w-3xl md:grid-cols-2 md:gap-4 lg:mt-8 lg:max-w-7xl lg:grid-cols-2 xl:grid-cols-4">
            {RELATED_TOOLS.map(({ href, titleKey, descKey }) => (
              <Link
                key={href}
                href={href}
                className="group rounded-2xl border border-white/10 bg-slate-900/45 p-4 transition hover:border-teal-500/40 hover:bg-teal-500/5 md:p-5"
              >
                <h3 className={`${TYPE.stepTitle} group-hover:text-teal-200`}>{t(titleKey)} →</h3>
      ...
      <section id="faq" aria-labelledby="faq-title" className="bg-slate-950">
        <div className={`${SECTION} ${SECTION_PY}`}>
          ...
          <dl className="mx-auto mt-5 max-w-xl divide-y divide-white/10 rounded-2xl border border-white/10 bg-slate-900/45 md:mt-7 md:max-w-3xl lg:mt-8 lg:max-w-7xl">
            {SHORTS_TO_WEBM_FAQ_ITEMS.map((item) => (
